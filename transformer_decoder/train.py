...
# In[4]:
'''
    def __init__(self, feature_size=2080, side_feature_size = 5, num_layers=3, dropout=0.1):
...
class TransAm(nn.Module):
    ...
    def __init__(self, feature_size=528, side_feature_size = 5, num_layers=3, dropout=0.1):
        super(TransAm, self).__init__()
        self.model_type = 'Transformer'
        ...
        self.tgt_mask = None
        self.pos_encoder = PositionalEncoding(feature_size)
        self.decode_pos_encoder = PositionalEncoding(feature_size+side_feature_size)
        print("Encoder")
        # Encoder
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=feature_size, nhead=8, dropout=dropout)
...
def train(train_dataset):
    ...
    for i, (idx, inputVar, targetVar) in enumerate(train_data_loader):
        ...
        # print(inputs.shape) # torch.Size([64, 7, 528])
        # print(label.shape) # torch.Size([64, 1, 528])
        optimizer.zero_grad()
        ...
        log_interval = int(len(train_dataset) / batch_size / 3) # 2874/ 64 / 5
...
def uniform_evaluation(pred, label, feature_size = 528, maps = None):
    pred = pred.detach()
    label = label.detach()
    diff = pred - label
